...
getModels("http://www.automobile-catalog.com/list-abarth.html", function(err, models){
	if (err){
		...
		if (!fs.existsSync(__dirname + "/" + "Abarth")) {
		    fs.mkdirSync(__dirname + "/" + "Abarth");
		}
						...
						getTrims(submodels[j].url, function(err, trims){
							if (err){
								...
								for (var k = 0; k < trims.length; k++){
									console.log(trims[k]);
									if (!fs.existsSync(__dirname + "/" + "Abarth" + "/" + trims[k].trim.replace(/\//g, " ").replace(/\\/g, " "))) {
									    fs.mkdirSync(__dirname + "/" + "Abarth" + "/" + trims[k].trim.replace(/\//g, " ").replace(/\\/g, " "));
									}
								}
							}
						});
